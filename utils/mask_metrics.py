...
def calculate_all_pairwise_metrics_ultra_optimized(mask_dir: str, num_objects: int, max_workers: int = None, batch_size: int = 50) -> dict:
    """
    Ultra-optimized version using ProcessPoolExecutor for true parallel processing.
    ...
    if max_workers is None:
        max_workers = min(cpu_count(), 8)  # Limit to 8 to avoid memory issues
    ...
    print(f"ULTRA-OPTIMIZED: Processing {len(frame_indices)} frames with {num_objects} objects")
    print(f"Using {max_workers} processes, batch size: {batch_size}")
    ...
    def process_batch(batch_data):
        ...
        batch_frames, mask_dir_str, num_objects = batch_data
        batch_metrics = {}
        
        # Load all masks for this batch at once
        batch_masks = load_batch_masks_ultra_optimized(Path(mask_dir_str).parent / 'soft_masks', batch_frames, num_objects)
        
        # Process each frame in the batch
        for frame_idx in batch_frames:
            if frame_idx in batch_masks:
                frame_metrics = calculate_frame_metrics_ultra_optimized(batch_masks[frame_idx], num_objects)
                batch_metrics[frame_idx] = frame_metrics
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Prepare batch data
        batch_data = [(batch, str(mask_dir), num_objects) for batch in frame_batches]
        
        # Submit all batch processing tasks
        future_to_batch = {executor.submit(process_batch, data): i for i, data in enumerate(batch_data)}
...
def load_batch_masks_ultra_optimized(soft_mask_dir: Path, frame_indices: List[int], num_objects: int) -> Dict[int, Dict[int, np.ndarray]]:
    """
    Ultra-optimized batch loading with memory mapping and aggressive caching.
    ...
    # Pre-check which files exist and get their sizes
    existing_files = {}
    for frame_idx in frame_indices:
        existing_files[frame_idx] = {}
        for obj_id in range(1, num_objects + 1):
    ...
    # Load masks using optimized I/O
    for frame_idx in frame_indices:
        frame_masks = {}
        for obj_id in range(1, num_objects + 1):
            if obj_id in existing_files[frame_idx]:
                ...
                try:
                    ...
                    mask = cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE)
                    if mask is not None:
                        # Convert to binary immediately and use uint8 for memory efficiency
                        frame_masks[obj_id] = (mask > 127).astype(np.uint8)
                except Exception as e:
                    print(f"Warning: Failed to load mask {mask_path}: {e}")
                    continue
...
def calculate_frame_metrics_ultra_optimized(object_masks: Dict[int, np.ndarray], num_objects: int) -> np.ndarray:
    """
    Ultra-optimized frame metrics calculation using vectorized operations.
    ...
    available_objects = list(object_masks.keys())
    
    if len(available_objects) < 2:
        return metrics
    
    # Pre-calculate all centroids and areas for efficiency
    centroids = {}
    areas = {}
    for obj_id in available_objects:
        ...
        if len(x_coords) > 0:
            centroids[obj_id] = (np.mean(x_coords), np.mean(y_coords))
            areas[obj_id] = len(x_coords)
        else:
            centroids[obj_id] = (0, 0)
            areas[obj_id] = 0
    ...
    for i in range(1, num_objects + 1):
        for j in range(i + 1, num_objects + 1):
            if i in available_objects and j in available_objects:
                mask1 = object_masks[i]
                mask2 = object_masks[j]
                
                # Calculate metrics using ultra-optimized function
                pair_metrics = calculate_pairwise_metrics_ultra_optimized(mask1, mask2, centroids[i], centroids[j])
...
def calculate_pairwise_metrics_ultra_optimized(mask1: np.ndarray, mask2: np.ndarray, 
                                             centroid1: Tuple[float, float], 
                                             centroid2: Tuple[float, float]) -> dict:
    """
    Ultra-optimized pairwise metrics calculation with pre-calculated centroids.
    
    Args:
        mask1: First binary mask
        mask2: Second binary mask
        centroid1: Pre-calculated centroid of first mask (x, y)
        centroid2: Pre-calculated centroid of second mask (x, y)
    ...
    # Calculate overlap ratio using optimized operations
    # Use logical operations directly on uint8 arrays
    intersection = np.logical_and(mask1, mask2).sum()
    union = np.logical_or(mask1, mask2).sum()
    overlap_ratio = intersection / union if union > 0 else 0.0
    
    # Calculate contact length using optimized dilation
    # Use smaller kernel and single iteration for speed
    kernel = np.ones((3, 3), np.uint8)
    dilated1 = cv2.dilate(mask1, kernel, iterations=1)
    dilated2 = cv2.dilate(mask2, kernel, iterations=1)
...
def calculate_all_pairwise_metrics_mega_optimized(mask_dir: str, num_objects: int) -> dict:
    """
    Mega-optimized version that uses the most aggressive optimizations.
    ...
    max_workers = min(cpu_count(), 12)  # Cap at 12 to avoid memory issues
    batch_size = 100  # Large batch size for better I/O efficiency
    
    print(f"MEGA-OPTIMIZED: Using {max_workers} processes, batch size: {batch_size}")
    
    return calculate_all_pairwise_metrics_ultra_optimized(mask_dir, num_objects, max_workers, batch_size) 